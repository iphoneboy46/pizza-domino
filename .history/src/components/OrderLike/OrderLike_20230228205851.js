...
function OrderLike() {
  const {menuOrders,setMenuOrders,quantity} = useContext(Context)  
...
const handleAddCart = (data,name) =>{
    ...
    alertAdded.classList.style.display = "block";
    setTimeout(()=>{
        alertAdded.classList.style.display = "none";
    },2000)
  ...
  return (
    <div className="border-[1px] border-[#0078AE] w-[100%] rounded-md py-5 px-2 relative overflow-hidden">
      ...
      <div className="lg:w-[900px] md:w-[302%] w-[300%] mb-3 overflow-hidden">
        <div id="list-order" className=" flex justify-around items-start w-[900%]">
          {listOrderLikes.map((like) => {
            if (like.id <= 15) {
              return (
                <div className="list-items flex lg:justify-center lg:items-center lg:flex-col md:flex-row md:justify-around md:items-center md:w-[500px] w-[500px]">
                  <div className="lg:w-[195px] h-[130px] md:w-[100px] w-[100px] rounded-md mb-3 mr-3">
                   ...
                   <div>
                       ...
                       }} className="border-[1px] border-[#E31837] lg:w-[200px] md:w-[200px] w-[150px] m-auto p-1 font-bold text-[#E31837] text-center cursor-pointer hover:bg-[#E31837] hover:text-white duration-150 ease-linear">
                       Thêm vào giỏ hàng
                       </div>
                   </div>
                </div>
