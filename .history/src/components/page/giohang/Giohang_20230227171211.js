...
function Giohang() {
    ...
    if (menuName) {
      setMenuOrders(
        menuOrders.map((menuOrder) => {
          let totalNumberCon = menuOrder.price * menuOrder.quantity;
          let numberFirst = totalNumberCon.toFixed().split("").slice(0, 1);
          let numberLast = totalNumberCon.toFixed().split("").slice(1).join("");
          const totalNumberTongP = numberFirst + "." + numberLast;

          return menuOrder.name === name
            ? {
                ...menuOrder,
                quantity: (menuOrder.quantity = menuOrder.quantity + 1),
                totalP: totalNumberCon > 1000 ? totalNumberTongP : totalNumberCon
  ...
  useEffect(() => {
    ...
    if (totalNumberCon > 1000) {
      const numberFirst = totalNumberCon.toFixed().split("").slice(0, 1);
      const numberLast = totalNumberCon.toFixed().split("").slice(1).join("");

      const totalNumberTongP = numberFirst + "." + numberLast;
      setTotalNumberCon(totalNumberTongP);
    }

    if (totalNumberCon > 10000) {
      const numberFirst = totalNumberCon.toFixed().split("").slice(0, 2).join("");
      const numberLast = totalNumberCon.toFixed().split("").slice(2).join("");
