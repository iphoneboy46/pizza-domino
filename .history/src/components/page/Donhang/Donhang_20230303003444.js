...
function Donhang() {
  ...
  const {phoneFilter,setPhoneFilter} = useContext(Context)

  useEffect(() => {
    // const phoneNumber = Number(number);
    console.log(number);

    phoneRef2.current.onkeyup = () => {
      if (number.trim() === "") {
        ...
        textAlert3.style.display = "block";
      } else {
        textAlert.style.display = "none";
        textAlert2.style.display = "none";
        textAlert3.style.display = "none";
        textAlert4.style.display = "none";
      }
    };
  }, [number]);

  const handleSub = () =>{
    setPhoneFilter(number)
  }
