...
function Donhang() {
  ...
  console.log(inputNumber.current.length);
  const checkValid = () =>{
      if(inputNumber.current <= 0){
        console.log(123);
      }
  }
  checkValid();
    ...
    <div className="flex items-center w-[40%] h-[80vh] justify-center m-auto">
      ...
      <div className="w-[80%]">
        ...
        <div className="bg-[white] flex justify-center rounded w-[100%] m-auto mt-5">
          <input
            type="text"
            className="py-[12px] px-[8px] lg:w-[100%] md:w-[80%] outline-[#80bdff] text-[#495057] font-extrabold rounded-l-lg  shadow-sm shadow-[#979797]"
            ref={inputNumber}
        ...
        <span className="text-[#DC3545] font-bold">Vui lòng nhập số điện thoại của bạn!</span>
      </div>
    </div>
  )
}
