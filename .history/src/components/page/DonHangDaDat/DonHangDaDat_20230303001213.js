...
function DonHangDaDat() {
  const { listDonHang,totalOrders } = useContext(Context);
  ...
  return (
    <div className="pt-[80px] bg-[#EEEEEE] h-[100vh]">
      ...
      <div className="bg-[#00628F] lg:w-[50%] md:w-[90%] w-[100%] m-auto h-[80vh] p-10 mb-10 overflow-y-scroll">
        {listDonHang.map((donHang) => {
          return (
            <div className="bg-white lg:w-[80%] md:w-[100%] w-[100%] m-[auto]">
              ...
              {donHang.data.map((menu) => {
               ...
               <div className="w-[100%] p-5 flex justify-between border-b-[1px] border-[lightgray] pb-2">
                ...
                <span className="text-end text-xl text-[red] font-bold">{donHang.tong > 1000 ? donHang.tong.toFixed().split("").slice(0, 1) + "." + donHang.tong.toFixed().split("").slice(1) : donHang.tong}.000 ₫</span>
              </div>
            </div>
          );
        })}
