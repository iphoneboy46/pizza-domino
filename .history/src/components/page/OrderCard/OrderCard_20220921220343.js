...
function OrderCard() {
  const { menuOrders,totalQuantity } = useContext(Context);
        ...
        {menuOrders.map((menu) => {
          return (
            <div className="mb-3">
              ...
              <div className="flex justify-center mr-[160px] py-2 cursor-pointer">
                <span className="mr-5  text-[#0078ae] text-sm font-bold">Điều chỉnh</span>
                <span className="text-[#0078ae] text-sm font-bold">Xóa</span>
              </div>
            </div>
          );
        })}
