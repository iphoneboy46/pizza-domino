...
function Thucdon() {
  ...
  const { menuOrders, address, orderAddress } = useContext(Context);
  console.log(activeFilter);

  const checkedAddress = orderAddress.find((order)=>{
    return order.address.value === address
  })
  ...
  return (
    <div className="pt-[80px] flex h-[670px] overflow-hidden">
           ...
           address  ?
            <div className="bg-[#e3f4fc]">
              <div className="flex items-center p-4 ml-[100px] text-[#6a6b6c] font-semibold">
                ...
                <h1 className="mr-5 text-[#de1736]">{orderAddress.takeAway || orderAddress.deliver}</h1>
