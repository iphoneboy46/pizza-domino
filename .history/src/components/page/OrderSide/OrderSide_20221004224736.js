...
function OrderSide() {
  const { orderSideDishes, quantity, setQuantity, address, inputRef,setMenuOrders,menuOrders } =
    useContext(Context);
  ...
  const handleOrder2 = (orderSideDishes) => {
    ...
    const orderSideDishesId = setMenuOrders.map((menu) =>{
        return menu.id === orderSideDishes.id
    })
    ...
    if (address) {
      ...
      if(orderSideDishesId){
        setMenuOrders(
          menuOrders.map((menu) =>{
              return menu.name === orderSideDishesId.name ? {...menu,...orderSideDishes,quantity: menu.quantity +1} : menu
          })
        )
      }else{
        setMenuOrders([...menuOrders,{...orderSideDishes,quantity:quantity}])
      }
  ...
  return (
    <div className="order-SideDishes w-[800px] h-[calc(100vh-400px)] bg-white flex m-auto fixed -top-[100%] left-[50%] -translate-x-[50%] rounded-[6px] ease-in duration-500 z-[9999]">
      ...
      <div className="w-[335px] h-[100%] rounded-l-lg overflow-hidden">
        <img
