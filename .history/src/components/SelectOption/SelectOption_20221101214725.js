...
function SelectOption(props) {
  ...
  const [place, setPlace] = useState("Chọn Tỉnh/Thành phố");
  ...
  useEffect(() => {
    function checkAddress() {
      if (address.includes("Hồ Chí Minh")) {
        setPlace("Hồ Chí Minh");
        setOptions("1");
       const optp =  optionTP.filter((option) => {
          return option.label === "Hồ Chí Minh";
        });
        console.log(optp)
