...
function SelectOption() {
  ...
  const [optionQItem, setOptionsQItem] = useState("null");
  const [itemBC, setItemBC] = useState(optionQHCM);

  console.log(optionQItem);

  useEffect(() => {
    const bc = itemBC.filter((bc)=>{
      return bc.value === optionQItem
    });
  },[optionQItem])
  ...
  return (
    <div className="w-[20%] p-3 bg-white">
      ...
      <div className="mb-5">
        ...
        {optionQ === "1" ? (
          <Select placeholder="Chọn Quận/Huyện" options={optionQHCM}  onChange={(e) => {
          setOptionsQItem(e.value);
        }} />
        ) : (
          ""
        )}
