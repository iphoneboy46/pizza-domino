...
function Header() {
  ...
  const { btnExits, setbtnExits } = useContext(Context);
  ...
  const handleLogin = () => {
    const scrollY = window.addEventListener("scroll", (e) => {
        var scrool = this.scrollY
    });
    console.log(scrollY);
    const loginForm = document.querySelector(".login-form");
    const offsetTop = loginForm.offsetTop + "px";
    console.log(scrollY);

    loginForm.style.top = `calc(50vh + ${offsetTop})`;
  };
      ...
      <div className="bg-[#004666] lg:flex md:flex p-[16px] w-[100%] h-[80px] items-center justify-between fixed z-[2000] overflow-hidden hidden">
        ...
        <div className="flex items-center">
          ...
          <div className="relative flex items-center">
            <button
              onClick={handleLogin}
              className="hover:bg-[#00628f] w-[100%] h-[100%] hover:text-wh mr-5"
            >
              <i class="fa-solid fa-user text-white text-4xl px-[10px] py-[20px]"></i>
            </button>
            <Link
              to="/cart"
              className="hover:bg-[#00628f] w-[100%] hover:text-wh mr-5"
            >
              <i class="fa-solid fa-basket-shopping text-white text-4xl  px-[10px] py-[20px]"></i>
              <span className="bg-[#aa1934] absolute px-2 font-bold text-white right-[25px] top-[15px] rounded">
